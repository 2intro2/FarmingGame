...
export default class NoodleChallengePage {
  ...
  dialog = null
  options = []
  correctKey = 'youche' // 正确答案：耧车

  panelRect = { x: 0, y: SCREEN_HEIGHT * 0.7, w: SCREEN_WIDTH, h: SCREEN_HEIGHT * 0.3 } // 记录黄色面板位置

  // 进度条（移动条）数据（固定前两格填充，第二格为当前段）
  progress = {
    x: 60,
    y: 82, // 调整后位置
    ...
    current: 1,     // 当前在第二段（黑框）
    activeFill: '#FFD54F',
    activeBorder: '#E6A700', // 深黄色边框（比填充更深）
    filledBorder: '#D18F00', // 已填充非当前段边框（比activeFill更深）
  ...
  loadResources() {
    ...
    this.backgroundImage.src = 'images/lbg01.jpeg'
    ...
    this.farmerImage.src = 'images/lbg01.jpeg'
    ...
    // 结果图
    this.passImage = wx.createImage()
    this.passImage.src = 'images/pass.png'

    this.failedImage = wx.createImage()
    this.failedImage.src = 'images/failed.png'
  ...
  render(ctx) {
    ...
    // 若开启过场并且正在播放，遮罩
    if (CUTSCENE_ENABLED && this.playingCutscene) {
    ...
    if (this.result.type === 'pass') {
      this.renderPassLayer(ctx)
    } else if (this.result.type === 'failed') {
      this.renderFailedLayer(ctx)
    }
  }
  ...
  renderProgressBar(ctx) {
    ...
    for (let i = 0; i < p.segments; i++) {
      ...
      this.roundRect(ctx, sx, p.y, segW, p.height, r)
  ...
  renderOptionsPanel(ctx) {
    ...
    ctx.font = '20px Arial'
    ctx.textAlign = 'left'
    ctx.fillText('请选择正确播种农具播种', 30, y + 30)
  }
  ...
  handleTouch(event) {
    ...
    if (this.result.type === 'pass') {
      ...
      if (CUTSCENE_ENABLED) {
        this.playCutsceneAndGotoNext()
      } else {
        GameGlobal.pageManager.switchToPage('noodleChallenge2')
      }
      return
    }
  ...
  // 保留过场相关方法，但受开关控制，不会被调用
  playCutsceneAndGotoNext() {
    if (!CUTSCENE_ENABLED) {
      GameGlobal.pageManager.switchToPage('noodleChallenge2')
      return
    }
    try {
      const video = wx.createVideo({
        ...
        src: 'images/animation.mp4', controls: false, autoplay: false,
        showCenterPlayBtn: false, enableProgressGesture: false,
        loop: false, muted: false, objectFit: 'contain', initialTime: 0
      })
      this.videoContext = video
      this.playingCutscene = true
      video.onEnded && video.onEnded(() => { this.stopAndRemoveVideo(); GameGlobal.pageManager.switchToPage('noodleChallenge2') })
      video.onError && video.onError(() => { this.stopAndRemoveVideo(); GameGlobal.pageManager.switchToPage('noodleChallenge2') })
  ...
  stopAndRemoveVideo() {
    try {
      if (this.videoContext) {
        ...
        this.videoContext.destroy && this.videoContext.destroy()
      }
    } catch (_) {}
