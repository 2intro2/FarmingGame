...
export default class NoodleChallengePage4 {
  ...
  initOptions() {
    ...
    this.options = [
      { key: 'quyuanni', label: '曲辕犁', x: baseX + (btnWidth + gap) * 0, y: areaY, width: btnWidth, height: 64 },
      { key: 'youche',   label: '水车',   x: baseX + (btnWidth + gap) * 1, y: areaY, width: btnWidth, height: 64 },
      { key: 'lianjia',  label: '连枷',   x: baseX + (btnWidth + gap) * 2, y: areaY, width: btnWidth, height: 64 },
      { key: 'liandao',  label: '石磨',   x: baseX + (btnWidth + gap) * 3, y: areaY, width: btnWidth, height: 64 },
    ]
  }
  ...
  renderOptionsPanel(ctx) {
    ...
    ctx.font = '20px Arial'
    ctx.textAlign = 'left'
    ctx.fillText('请选择正确的工具进行磨面', 30, y + 30)
  }
  ...
  handleTouch(event) {
    ...
    if (this.result.type === 'pass') {
      ...
      // 第四关通过后跳转到结束页面
      GameGlobal.pageManager.switchToPage('noodleEnd')
      return
    }
